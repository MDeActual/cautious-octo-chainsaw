    ...
    const checkHealth = async (service: ServiceStatus): Promise<ServiceStatus> => {
      try {
        const res = await fetch(
          `${service.name === 'identity-service' ? (import.meta.env['VITE_IDENTITY_API'] ?? `http://localhost:${service.port}`) : (import.meta.env['VITE_CORE_API'] ?? `http://localhost:${service.port}`)}/health`,
        );
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <header className="bg-gray-800 border-b border-gray-700 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <span className="text-gray-400 text-sm">{account?.username ?? 'Unknown user'}</span>
          <button
